...
export default async function compareImages(
  ...
  if (
    !oldImageFileNames.filter((name) => name !== 'diff' && name !== 'temp')
      .length
  ) {
  ...
  const bar = new ProgressBar(
    `(:current/:total) [:bar] ${chalk.blue(':newImageFileName')}`,
    {
  ...
  await pMap(
    ...
    async (newImageFileName) => {
      ...
      if (!oldImageExists) {
        if (!interactive)
          exitWithError(`Could not find ${newImageFileName} in ${oldPath}`)
        console.log('\n')
        const answer = await inquirer.prompt({
          type: 'confirm',
        ...
        await fs.copyFile(
          path.join(newPath, newImageFileName),
          path.join(oldPath, newImageFileName)
        )
        return
      }
      ...
      const newImagePath = path.join(newPath, newImageFileName)
      const [oldImage, newImage] = await loadImage.all([
        oldImagePath,
        newImagePath,
      ])
